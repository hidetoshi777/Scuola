(function () {
  ...
  function renderBins() {
    ...
    data.bins.forEach((bin) => {
      ...
      box.className = `tri-bin lane-bin lane-bin--${bin.id}`;
  ...
  function renderPool() {
    pool.innerHTML = "";
    const items = window.mescola ? window.mescola(data.casi) : data.casi;
    items.forEach((g) => {
  ...
  function renderPlaced() {
    ...
    Object.entries(placement).forEach(([id, binId]) => {
      const g = data.casi.find((x) => x.id === id);
      if (!g) return;
  ...
  function check() {
    const total = data.casi.length;
    const placed = Object.keys(placement).length;
    if (placed < total) {
      feedback.hidden = false;
      feedback.className = "feedback is-bad";
      feedback.textContent = `Ne mancano ${total - placed}. Metti tutte le schede nelle tre scuole.`;
      return;
    }
    ...
    grid.querySelectorAll(".chip.is-placed").forEach((chip) => {
      ...
      const g = data.casi.find((x) => x.id === chip.dataset.id);
    ...
    if (giusti === total) {
      feedback.textContent = "Perfetto: le tre scuole sono a posto. Ora prova il percorso sul volgare.";
      if (window.AudioUi) window.AudioUi.beep("win");
    } else {
      const errori = data.casi.filter((g) => placement[g.id] !== g.bin);
