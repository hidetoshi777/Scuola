(function () {
  const storia = window.STORIA_EDISON;
  if (!storia) {
    return;
  }
  ...
  const KEY = "edison-guerra-correnti-nodo";
  ...
  function render(id) {
    ...
    const scenaNuova = nodo.scena || "intro";
    ...
    if (window.AudioUi) {
      if (nodo.epilogo) {
        ...
        if (scenaNuova === "chicago" || scenaNuova === "rivale") {
          window.AudioUi.beep("scene");
        } else if (scenaNuova === "menlo" || scenaNuova === "kineto") {
  ...
  function scenaLabel(scena) {
    const map = {
      intro: "Dopo il film",
      menlo: "Menlo Park · laboratorio",
      manhattan: "Manhattan · CC",
      rivale: "Westinghouse e Tesla",
      chicago: "Chicago 1893 · Expo",
      kineto: "Kinetoscopio",
    };
