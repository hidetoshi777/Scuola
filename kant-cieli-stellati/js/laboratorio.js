(function () {
  ...
  function renderPool() {
    pool.innerHTML = "";
    const items = window.mescola ? window.mescola(data.giudizi) : data.giudizi;
    items.forEach((g) => {
  ...
  function renderPlaced() {
    ...
    Object.entries(placement).forEach(([id, binId]) => {
      const g = data.giudizi.find((x) => x.id === id);
      if (!g) return;
  ...
  function check() {
    const total = data.giudizi.length;
    const placed = Object.keys(placement).length;
    if (placed < total) {
      feedback.hidden = false;
      feedback.className = "feedback is-bad";
      feedback.textContent = `Ne mancano ${total - placed}. Metti tutte le schede nei quadranti.`;
      return;
    }
    ...
    grid.querySelectorAll(".chip.is-placed").forEach((chip) => {
      ...
      const g = data.giudizi.find((x) => x.id === chip.dataset.id);
    ...
    if (giusti === total) {
      feedback.textContent = "Perfetto: i quadranti sono a posto. Ora prova la passeggiata.";
      if (window.AudioUi) window.AudioUi.beep("win");
    } else {
      const errori = data.giudizi.filter((g) => placement[g.id] !== g.bin);
